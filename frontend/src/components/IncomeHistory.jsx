...
const IncomeHistory = () => {
  ...
  const [stats, setStats] = useState({ total: 0, pending: 0, thisMonth: 0, average: 0 });
  ...
  const fetchIncome = useCallback(async () => {
    ...
    const average = data.stats.average || (data.transactions.length > 0 ? Math.round(data.stats.total / data.transactions.length) : 0);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          ...
          <h3 className="text-gray-500 text-sm font-medium">Moyenne / Résa</h3>
          <p className="text-2xl font-bold text-gray-900 mt-1">{stats.average.toLocaleString()} FCFA</p>
        </div>
      </div>
