...
class Booking(models.Model):
    ...
    total_price = models.DecimalField(max_digits=12, decimal_places=2, verbose_name=_("Prix total"), blank=True)
    ...
    def clean(self):
        ...
        if self.start_time and self.end_time:
            ...
            # Overlap check for confirmed/paid bookings
            overlapping_bookings = Booking.objects.filter(
                property=self.property,
                start_time__lt=self.end_time,
                end_time__gt=self.start_time
            ).exclude(pk=self.pk).exclude(status='CANCELLED').exclude(status='FAILED')
    ...
    def calculate_total_price(self):
        ...
        """
        if self.start_time and self.end_time and self.property:
