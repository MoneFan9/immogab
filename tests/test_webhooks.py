...
def mock_session(mocker):
    mock_sess = MagicMock()
    mocker.patch("immogab.services.get_jeedom_session", return_value=mock_sess)
    return mock_sess

def test_jeedom_webhook_success(mock_session):
    # Setup mock response
...
def test_jeedom_webhook_rpc_error(mock_session):
...
def test_jeedom_webhook_401_unauthorized(mock_session):
    mock_response = MagicMock()
...
def test_jeedom_webhook_500_server_error(mock_session):
    mock_response = MagicMock()
...
def test_jeedom_webhook_timeout(mock_session):
    mock_session.post.side_effect = requests.exceptions.Timeout

    with pytest.raises(ConnectionError, match="Jeedom connection timed out"):
        call_jeedom_webhook("http://jeedom.local/api", "lock_cmd", "key")

def test_jeedom_webhook_other_error(mock_session):
    mock_response = MagicMock()
...
def test_jeedom_webhook_request_exception(mock_session):
    mock_session.post.side_effect = requests.exceptions.RequestException("Network fail")
